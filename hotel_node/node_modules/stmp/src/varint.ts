...
export function readVarint(buf: Buffer): number | null {
  let value: number;
...
export function varintLength(value: number) {
...
export const MAX_VARINT_VALUE = +0xfffffff;
export const MIN_VARINT_VALUE = -0xfffffff;

export function writeVarint(value: number, buf?: Buffer) {
  const negative = value < 0;
  value = Math.abs(value);
  if (value > MAX_VARINT_VALUE) {
    throw new RangeError(
      `out of varint range, from -${MAX_VARINT_VALUE} to ${MAX_VARINT_VALUE}`,
    );
  }
  if ((value | 0) !== value) {
  ...
  for (let i = length - 1; i > -1; i--) {
    if (i === 3) {
      ...
      buf.writeUInt8(
        value |
          (negative ? (length > 1 ? 0b11000000 : 0x80) : length > 1 ? 0x40 : 0),
        i,
      );
    } else {
