<template>
  <el-form :inline="true" class="demo-form-inline">
    <el-form-item>
      <el-input v-model="formInline.code" placeholder="员工号"></el-input>
    </el-form-item>
    <el-form-item>
...
export default {
  ...
  methods: {
    ...
    deleteRow(index, rows) {
      ...
      this.$alert('请确定是否删除员工', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(()=> {
        this.$axios({
          url:'http://localhost:9000/manager/staff',
          method:"delete",
          data:{
  ...
  created() {
    ...
    self.$axios({
      method:'get',
      url:'http://localhost:9000/manager/'+this.currentPage,
    }).then(res=>{
      var data=res.data
      this.total=data.a
      this.tableData=data.dataList
